...
import com.example.flowdesk_android.feature.user_management.domain.model.CatalogPage
import com.example.flowdesk_android.feature.user_management.domain.model.CatalogPermission
...
class ManagePermissionsAdapter(
    ...
    private var catalog: PermissionCatalog? = null

    fun submitCatalog(newCatalog: PermissionCatalog, initialCheckedIds: Set<Int>) {
        this.catalog = newCatalog
        checkedIds.clear()
        checkedIds.addAll(initialCheckedIds)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val page = catalog?.pages?.get(position) ?: return
        val permissions = catalog?.permissions?.filter { it.pageId == page.pageId } ?: emptyList()
        holder.bind(page, permissions)
    }
    ...
    inner class ViewHolder(private val binding: ItemRoleManagePermissionGroupBinding) :
        ...
        private fun selectAllListener(
            actions: List<CatalogPermission>
